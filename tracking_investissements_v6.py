...
def display_create_portfolio_button():
    ...
    # Afficher le bouton seulement si connecté ET 2FA validé ET aucun fichier chargé
    if is_authenticated and is_2fa_verified and not has_loaded_file:
        st.sidebar.markdown("---")
...
st.set_page_config(page_title='TLB INVESTOR Portfolio', layout='wide')
...
# === GESTION DE L'AUTHENTIFICATION AVEC 2FA ===
try:
    with st.container():
        authenticator.login()
...
# === TRAITEMENT DES CAS D'AUTHENTIFICATION ===
if st.session_state.get('authentication_status') is False:
    st.error('Nom d\'utilisateur ou mot de passe incorrect')
    clear_all_user_data()  # Nettoyer en cas d'échec de connexion
    
elif st.session_state.get('authentication_status') is None:
    # Page de connexion - afficher aide mot de passe oublié
    ...
    # Importer le module 2FA
    try:
        from modules.auth_2fa import check_2fa_status, display_2fa_page, cleanup_2fa_session
    except ImportError as e:
        st.error(f"❌ Module 2FA non trouvé : {e}")
        st.info("Assurez-vous que le fichier 'modules/auth_2fa.py' existe.")
        st.stop()
    
    # Vérifier le statut 2FA
    tfa_status = check_2fa_status(username, config)
    
    if tfa_status == 'required':
        # === PAGE 2FA ===
        ...
        if not user_email:
            st.error(f"❌ Aucun email configuré pour l'utilisateur {username} dans config.yaml")
            st.info("Ajoutez un champ 'email' dans la configuration de cet utilisateur.")
            st.stop()
        
        # Afficher la page 2FA
        st.markdown("---")
        if display_2fa_page(username, user_email, user_name, config):
            # 2FA validée, recharger la page
            st.rerun()
        else:
            # 2FA non validée, arrêter ici
            st.stop()
    
    elif tfa_status == 'not_required':
        # Utilisateur exclu de la 2FA, marquer comme vérifié
        st.session_state.tlb_2fa_verified = True
    
    # === À CE POINT, L'UTILISATEUR EST COMPLÈTEMENT AUTHENTIFIÉ ===
    ...
    # Message de bienvenue avec indicateur 2FA
    if tfa_status == 'not_required':
        st.success(f"Bienvenue {st.session_state['name']} ! 🔓")
    else:
        st.success(f"Bienvenue {st.session_state['name']} ! 🔐 (2FA Vérifié)")
...
# === ARRÊT SI PAS CONNECTÉ OU 2FA NON VALIDÉ ===
if (st.session_state.get('authentication_status') is not True or 
    not st.session_state.get('tlb_2fa_verified', False)):
    st.stop()
